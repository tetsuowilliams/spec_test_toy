# Feature Specification: Count PDFs in a directory

**Feature Branch**: `001-count-pdfs-in-directory`  
**Created**: 2025-03-27  
**Status**: Draft  
**Input**: User description: "I want to create a simple command line application that given a directory will count the number of PDF files in it."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Get PDF count for a folder (Priority: P1)

A user wants to point a small command-line tool at a folder on their computer and learn how many
PDF documents sit **directly** in that folder (not inside subfolders), so they can verify
...
**Independent Test**: Create a disposable folder with a known mix of files (including PDFs and
non-PDFs, varied name casing, optional subfolder with an extra PDF). Run the tool against that folder
and verify the count matches only the PDFs that are immediate children of that folder, and that
behavior matches **Edge Cases** below for invalid inputs.

**Acceptance Scenarios**:

1. **Given** a directory that contains three files whose names end in `.pdf` (any letter casing) and
   other non-PDF files, **When** the user runs the tool with that directory, **Then** the user sees
   that the count is three.
2. **Given** an empty directory, **When** the user runs the tool with that directory, **Then** the
   user sees a count of zero.
3. **Given** a directory that has a subfolder containing additional PDF files, **When** the user
   runs the tool with the parent directory, **Then** PDFs inside the subfolder are not included in
   the count.
...
5. **Given** a directory that cannot be read (for example permission denied), **When** the user runs
   ...
   misleading PDF count.
6. **Given** a directory containing an entry whose name ends with `.pdf` but which is not a regular
   file (for example a subfolder named `archive.pdf`), **When** the user runs the tool, **Then** that
   entry is not included in the PDF count.
7. **Given** a directory that includes a regular file named like `.report.pdf` (leading dot), **When**
...
8. **Given** a directory path that contains spaces or non-ASCII characters and that the user’s
   ...
   count matches the number of in-scope PDF files in that directory.
...
- The directory contains no PDFs, only PDFs, or a mix including hidden names (e.g., `.report.pdf`).
- File names use different casing for the extension (`.pdf`, `.PDF`, `.Pdf`).
- Names look like PDFs but are not regular files (e.g., directories or special entries)—the tool
  MUST NOT treat those as PDF files for counting.
...
- **FR-002**: The tool MUST count only entries in that directory that are PDF documents for this
  feature: regular files whose name ends with `.pdf` in a case-insensitive way.
- **FR-003**: The tool MUST NOT include PDFs (or any files) located only inside descendant
...
- **FR-004**: On success, the tool MUST present the count to the user in an unambiguous way (a single
  numeric result for the PDF total).
...
- **Target directory**: The folder the user specifies; the scope of scanning is only its immediate
  children.
- **PDF file (in scope)**: A regular file in that directory whose filename has a case-insensitive
  `.pdf` suffix.
...
- **SC-001**: For any prepared directory where the correct number of in-scope PDF files is known by
...
- **SC-003**: In failure scenarios defined in **FR-005**, one hundred percent of exercise runs must
  produce a clear error indication (message or non-success outcome), never a misleading PDF count.
...
- PDF detection is **by filename extension only** (`.pdf`, case-insensitive), not by inspecting
  file contents.
